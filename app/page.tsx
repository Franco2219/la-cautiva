      ...
      <div className={`w-full ${['ranking-view', 'group-phase', 'direct-bracket'].includes(navState.level) ? 'max-w-7xl' : 'max-w-6xl'} mx-auto z-10`}>
        ...
        <div className={`space-y-4 ${['ranking-view', 'group-phase', 'direct-bracket'].includes(navState.level) ? 'w-full' : 'max-w-xl mx-auto'}`}>
          ...
          {navState.level === "direct-bracket" && (
            <div className="bg-white border-2 border-[#b35a38]/10 rounded-[2.5rem] p-8 shadow-2xl overflow-x-auto animate-in fade-in duration-500 text-center">
                  ...
                  {[0, 2].map((idx) => {
                    ...
                    return (
                      <div key={idx} className="relative">
                        ...
                        <div className="absolute top-[71.5px] -right-[78px] w-8 h-[2px] bg-slate-300" />
                      </div>
                    )
                  })}
                </div>
                    ...
                    {[0, 1].map((idx) => {
                      ...
                      return (
                        <div key={idx} className="relative h-10 flex items-end">
                          <div className={`w-full border-b-4 ${win ? 'border-[#b35a38]' : 'border-slate-200'} pb-2 flex justify-between items-end bg-white z-10 text-center`}>
