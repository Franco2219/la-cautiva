...
export default function Home() {
  ...
  const fetchBracketData = async (category: string, tournamentShort: string) => {
    ...
    try {
      ...
      setBracketData({
        r1: rows.map(r => r[0]).slice(0, 8),   // Col A
        s1: rows.map(r => r[1]).slice(0, 8),   // Col B
        r2: rows.map(r => r[2]).slice(0, 4),   // Col C
        s2: rows.map(r => r[3]).slice(0, 4),   // Col D
        r3: rows.map(r => r[4]).slice(0, 2),   // Col E
        s3: rows.map(r => r[5]).slice(0, 2),   // Col F
        winner: rows[0][6] || ""               // Col G
      });
    } catch (error) {
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-4 relative bg-[#fffaf5]">
      <div className={`w-full ${['ranking-view', 'group-phase', 'direct-bracket'].includes(navState.level) ? 'max-w-7xl' : 'max-w-6xl'} mx-auto z-10`}>
        ...
        <div className={`space-y-4 ${['ranking-view', 'group-phase', 'direct-bracket'].includes(navState.level) ? 'w-full' : 'max-w-xl mx-auto'}`}>
          ...
          {navState.level === "year-selection" && <div className="space-y-4"><h2 className="text-2xl font-black text-center mb-4 text-slate-800 uppercase text-center">Temporada</h2><Button onClick={() => setNavState({ level: "category-selection", type: "ranking", year: "2025" })} className={buttonStyle}>Ranking 2025</Button><Button onClick={() => setNavState({ level: "category-selection", type: "ranking", year: "2026" })} className={buttonStyle}>Ranking 2026</Button></div>}
          {navState.level === "category-selection" && <div className="space-y-4"><h2 className="text-2xl font-black text-center mb-4 text-slate-800 uppercase text-center">{navState.type} {navState.year || ""}</h2>{["Categoría A", "Categoría B1", "Categoría B2", "Categoría C"].map((cat) => (<Button key={cat} onClick={() => { const catShort = cat.replace("Categoría ", ""); if (navState.type === "ranking") { fetchRankingData(catShort, navState.year); setNavState({ ...navState, level: "ranking-view", selectedCategory: cat }); } else { setNavState({ ...navState, level: "tournament-selection", category: catShort, selectedCategory: cat, gender: navState.type }); } }} className={buttonStyle}>{cat}</Button>))}</div>}
          {navState.level === "tournament-selection" && <div className="space-y-4 max-w-xl mx-auto"><h2 className="text-2xl font-black text-center mb-4 text-slate-800 uppercase text-center">{navState.selectedCategory}</h2>{tournaments.filter(t => { if (t.id === "s8_500") return ["B1", "B2", "C"].includes(navState.category); if (t.id === "s8_250") return ["B1", "B2"].includes(navState.category); return true; }).map((t) => (<Button key={t.id} onClick={() => { if (t.type === "direct" && navState.gender === "caballeros") { fetchBracketData(navState.category, t.short || ""); setNavState({ ...navState, level: "direct-bracket", tournament: t.name }); } else { setNavState({ ...navState, level: "tournament-phases", tournament: t.name }); } }} className={buttonStyle}>{t.name}</Button>))}</div>}
          ...
          {navState.level === "direct-bracket" && (
            <div className="bg-white border-2 border-[#b35a38]/10 rounded-[2.5rem] p-8 shadow-2xl overflow-x-auto animate-in fade-in duration-500">
              <div className="bg-[#b35a38] p-6 rounded-2xl mb-12 text-center italic min-w-[800px]"><h2 className="text-3xl md:text-5xl font-black text-white uppercase">{navState.tournament}</h2></div>
              <div className="flex flex-row items-center min-w-[950px] max-w-6xl mx-auto py-10 relative">
                ...
                <div className="flex flex-col space-y-16 w-72 relative z-10">
                  {[0, 2, 4, 6].map((idx) => {
                    const win1 = bracketData.r1[idx] && bracketData.r2.includes(bracketData.r1[idx]);
                    const win2 = bracketData.r1[idx+1] && bracketData.r2.includes(bracketData.r1[idx+1]);
                    return (
                      <div key={idx} className="relative">
                        <div className="space-y-6">
                          <div className={`border-b-2 ${win1 ? 'border-[#b35a38]' : 'border-slate-300'} pb-1 flex justify-between items-end bg-white relative`}>
                            <span className={`${win1 ? 'text-[#b35a38] font-black' : 'text-slate-700 font-bold'} uppercase text-[10px] truncate max-w-[150px]`}>{bracketData.r1[idx] || "TBD"}</span>
                            <span className="text-[#b35a38] font-black text-[9px] ml-2">{bracketData.s1[idx]}</span>
                          </div>
                          <div className={`border-b-2 ${win2 ? 'border-[#b35a38]' : 'border-slate-300'} pb-1 flex justify-between items-end bg-white relative`}>
                            <span className={`${win2 ? 'text-[#b35a38] font-black' : 'text-slate-700 font-bold'} uppercase text-[10px] truncate max-w-[150px]`}>{bracketData.r1[idx+1] || "TBD"}</span>
                            <span className="text-[#b35a38] font-black text-[9px] ml-2">{bracketData.s1[idx+1]}</span>
                          </div>
                        </div>
                        {/* Conexión Recta perfecta sin curvas */}
                        <div className="absolute top-[18px] -right-[48px] w-[48px] h-[54px] border-r-2 border-t-2 border-b-2 border-slate-300" />
                ...
                <div className="flex flex-col space-y-[158px] w-64 ml-24 relative z-10">
                  {[0, 2].map((idx) => {
                    const win1 = bracketData.r2[idx] && bracketData.r3.includes(bracketData.r2[idx]);
                    const win2 = bracketData.r2[idx+1] && bracketData.r3.includes(bracketData.r2[idx+1]);
                    return (
                      <div key={idx} className="relative">
                        <div className="space-y-10">
                          <div className={`border-b-2 ${win1 ? 'border-[#b35a38]' : 'border-slate-300'} pb-1 flex justify-between items-end h-6 bg-white`}>
                            <span className={`${win1 ? 'text-[#b35a38] font-black' : 'text-slate-700 font-bold'} uppercase text-[10px]`}>{bracketData.r2[idx]}</span>
                            <span className="text-[#b35a38] font-black text-[9px] ml-2">{bracketData.s2[idx]}</span>
                          </div>
                          <div className={`border-b-2 ${win2 ? 'border-[#b35a38]' : 'border-slate-300'} pb-1 flex justify-between items-end h-6 bg-white`}>
                            <span className={`${win2 ? 'text-[#b35a38] font-black' : 'text-slate-700 font-bold'} uppercase text-[10px]`}>{bracketData.r2[idx+1]}</span>
                            <span className="text-[#b35a38] font-black text-[9px] ml-2">{bracketData.s2[idx+1]}</span>
                          </div>
                        </div>
                        <div className="absolute top-[18px] -right-[48px] w-[48px] h-[70px] border-r-2 border-t-2 border-b-2 border-slate-300" />
                        <div className="absolute top-[53px] -right-[78px] w-8 h-[2px] bg-slate-300" />
                      </div>
                    )
                  })}
                </div>
                ...
                <div className="flex flex-col items-center ml-24 w-80 relative z-10">
                  <div className="w-full space-y-12 mb-16">
                    {[0, 1].map((idx) => {
                      const isWinner = bracketData.r3[idx] && bracketData.r3[idx] === bracketData.winner;
                      return (
                        <div key={idx} className={`border-b-4 ${isWinner ? 'border-[#b35a38]' : 'border-slate-200'} pb-2 h-8 bg-white flex justify-between items-end`}>
                          <span className={`${isWinner ? 'text-[#b35a38] font-black' : 'text-slate-800 font-bold'} uppercase text-xs`}>{bracketData.r3[idx]}</span>
                  ...
                  <div className="flex flex-col items-center animate-bounce duration-[3000ms]">
                    ...
                    <span className="text-[#b35a38] font-black text-3xl mt-2 italic uppercase tracking-tighter">{bracketData.winner || "Campeón"}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          {/* Torneos Regulares y Grupos */}
          {navState.level === "tournament-phases" && <div className="space-y-4 max-w-xl mx-auto text-center"><h2 className="text-2xl font-black mb-4 text-slate-800 uppercase">{navState.tournament}</h2><Button onClick={() => setNavState({ ...navState, level: "group-phase" })} className={buttonStyle}><Users className="mr-2" /> Fase de Grupos</Button><Button onClick={() => setNavState({ ...navState, level: "bracket-phase" })} className={buttonStyle}><Grid3x3 className="mr-2" /> Cuadro de Eliminación</Button></div>}
