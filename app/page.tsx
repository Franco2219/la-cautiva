    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-4 bg-[#fffaf5]">
      <div className={`w-full ${['ranking-view', 'group-phase', 'direct-bracket'].includes(navState.level) ? 'max-w-7xl' : 'max-w-6xl'} mx-auto z-10`}>
        ...
        <div className="text-center mb-8">
          <Image src="/logo.png" alt="Logo" width={180} height={180} className="mx-auto mb-4" priority />
        ...
        <div className={`space-y-4 ${['ranking-view', 'group-phase', 'direct-bracket'].includes(navState.level) ? 'w-full' : 'max-w-xl mx-auto'}`}>
          ...
          {/* ... Resto de la navegación intacta ... */}
          ...
          {navState.level === "direct-bracket" && (
            <div className="bg-white border-2 border-[#b35a38]/10 rounded-[2.5rem] p-8 shadow-2xl overflow-x-auto">
              <div className="bg-[#b35a38] p-6 rounded-2xl mb-12 text-center min-w-[800px]">
                <h2 className="text-3xl font-black text-white uppercase">{navState.tournament}</h2>
              </div>
              <div className="flex flex-row items-center min-w-[950px] max-w-6xl mx-auto py-10 relative">
                    ...
                    return (
                      <div key={idx} className="relative">
                        <div className="space-y-6">
                          <div className={`border-b-2 ${win1 ? 'border-[#b35a38]' : 'border-slate-300'} pb-1 flex justify-between items-end bg-white`}>
                            <span className={`${win1 ? 'text-[#b35a38] font-black' : 'text-slate-700 font-bold'} uppercase text-[10px]`}>{bracketData.r1[idx] || "TBD"}</span>
                            <span className="text-[#b35a38] font-black text-[9px]">{bracketData.s1[idx]}</span>
                          </div>
                          <div className={`border-b-2 ${win2 ? 'border-[#b35a38]' : 'border-slate-300'} pb-1 flex justify-between items-end bg-white`}>
                            <span className={`${win2 ? 'text-[#b35a38] font-black' : 'text-slate-700 font-bold'} uppercase text-[10px]`}>{bracketData.r1[idx+1] || "TBD"}</span>
                            <span className="text-[#b35a38] font-black text-[9px]">{bracketData.s1[idx+1]}</span>
                          </div>
                        </div>
                        {/* Conexión Recta */}
                        <div className="absolute top-[18px] -right-[48px] w-[48px] h-[54px] border-r-2 border-t-2 border-b-2 border-slate-300" />
                        <div className="absolute top-[45px] -right-[78px] w-[30px] h-[2px] bg-slate-300" />
                      </div>
                    )
                  })}
                </div>
                      ...
                      <div key={idx} className="relative">
                        <div className="space-y-10">
                          <div className={`border-b-2 ${win1 ? 'border-[#b35a38]' : 'border-slate-300'} pb-1 flex justify-between items-end h-6 bg-white`}>
                            ...
                            <span className="text-[#b35a38] font-black text-[9px]">{bracketData.s2[idx]}</span>
                          </div>
                          <div className={`border-b-2 ${win2 ? 'border-[#b35a38]' : 'border-slate-300'} pb-1 flex justify-between items-end h-6 bg-white`}>
                            ...
                            <span className="text-[#b35a38] font-black text-[9px]">{bracketData.s2[idx+1]}</span>
                          </div>
                        </div>
                        <div className="absolute top-[18px] -right-[48px] w-[48px] h-[70px] border-r-2 border-t-2 border-b-2 border-slate-300" />
                        <div className="absolute top-[53px] -right-[78px] w-[30px] h-[2px] bg-slate-300" />
                      </div>
                    )
                  })}
                </div>
                ...
                <div className="flex flex-col items-center ml-24 w-80 relative z-10">
                  <div className="w-full space-y-12 mb-16">
                    {[0, 1].map((idx) => {
                      const win = bracketData.r3[idx] && bracketData.r3[idx] === bracketData.winner;
                      return (
                        <div key={idx} className={`border-b-4 ${win ? 'border-[#b35a38]' : 'border-slate-200'} pb-2 flex justify-between items-end bg-white`}>
                          <span className={`${win ? 'text-[#b35a38] font-black' : 'text-slate-800 font-bold'} uppercase text-xs`}>{bracketData.r3[idx]}</span>
                          <span className="text-[#b35a38] font-black text-[10px]">{bracketData.s3[idx]}</span>
                        </div>
                      )
                    })}
                  </div>
                  <Trophy className="w-20 h-20 text-orange-400" />
                  <span className="text-[#b35a38] font-black text-4xl mt-2 italic uppercase">{bracketData.winner || "Campeón"}</span>
                </div>
              </div>
            </div>
          )}
