...
export default function Home() {
    ...
    matches.forEach((match, index) => {
        ...
        if (match.p1) {
            ...
            if (pool.length >= 2) {
                ...
                match.p1 = pool.pop(); match.p2 = pool.pop();

            } else if (pool.length === 1) {

                match.p1 = pool.pop(); match.p2 = { name: "BYE", rank: 0 };
  ...
  const fetchBracketData = async (category: string, tournamentShort: string) => {
    ...
    const processByes = (data: any) => {
        ...
        const newR2 = [...r2], newR3 = [...r3];

        if (isLarge) {

            for (let i = 0; i < r1.length; i += 2) {

                const p1 = r1[i], p2 = r1[i+1], targetIdx = Math.floor(i / 2);
        ...
        const roundPrev = isLarge ? newR2 : r1, roundNext = isLarge ? newR3 : r2;

        for (let i = 0; i < roundPrev.length; i += 2) {

             const p1 = roundPrev[i], p2 = roundPrev[i+1], targetIdx = Math.floor(i / 2);
        ...
        if (isLarge) { data.r3 = newR3; } else { data.r2 = roundNext; } return data;
    ...
    try {
      ...
      if (rows.length > 0 && firstCell !== "" && firstCell !== "-") {
          ...
          let rawData: any = isLarge ? { r1: rows.map(r => r[0]).slice(0, 32), s1: rows.map(r => r[1]).slice(0, 32), r2: rows.map(r => r[2]).slice(0, 16), s2: rows.map(r => r[3]).slice(0, 16), r3: rows.map(r => r[4]).slice(0, 8), s3: rows.map(r => r[5]).slice(0, 8), r4: rows.map(r => r[6]).slice(0, 4), s4: rows.map(r => r[7]).slice(0, 4), winner: rows[0][8] || "", isLarge: true, hasData: true, canGenerate: false, seeds } : { r1: rows.map(r => r[0]).slice(0, 16), s1: rows.map(r => r[1]).slice(0, 16), r2: rows.map(r => r[2]).slice(0, 8), s2: rows.map(r => r[3]).slice(0, 8), r3: rows.map(r => r[4]).slice(0, 4), s3: rows.map(r => r[5]).slice(0, 4), winner: rows[0][6] || "", isLarge: false, hasData: true, canGenerate: false, seeds };
        ...
        <div className="space-y-4 max-w-xl mx-auto">
          ...
          {navState.level === "tournament-selection" && (
            ...
            <div className="space-y-4 text-center">
              ...
              {tournaments.filter(t => !((t.id === "adelaide" && navState.gender === "damas") || ((t.id === "s8_500" || t.id === "s8_250") && navState.category === "A") || (t.id === "s8_250" && navState.category === "C"))).map((t) => (

                <Button key={t.id} onClick={() => { if (t.type === "direct") { fetchBracketData(navState.category, t.short); setNavState({ ...navState, level: "direct-bracket", tournament: t.name, tournamentShort: t.short }); } else { fetchGroupPhase(navState.category, t.short); } }} className={buttonStyle}>{t.name}</Button>
              ...
              <div className="flex flex-row items-center justify-between min-w-[1300px] py-2 relative text-center">
                ...
                <div className={`flex flex-col justify-around h-auto min-h-[600px] w-80 relative ${bracketData.isLarge ? 'ml-24' : ''} text-left`}>
                  ...
                  {Array.from({length: 8}, (_, i) => i * 2).map((idx) => {
                    ...
                    const p1 = bracketData.isLarge ? bracketData.r2[idx] : bracketData.r1[idx], p2 = bracketData.isLarge ? bracketData.r2[idx+1] : bracketData.r1[idx+1], w1 = p1 && (bracketData.isLarge ? bracketData.r3.includes(p1) : bracketData.r2.includes(p1)), w2 = p2 && (bracketData.isLarge ? bracketData.r3.includes(p2) : bracketData.r2.includes(p2));

                    const s1 = bracketData.isLarge ? bracketData.s2[idx] : bracketData.s1[idx], s2 = bracketData.isLarge ? bracketData.s2[idx+1] : bracketData.s1[idx+1];

                    const seed1 = bracketData.seeds ? bracketData.seeds[p1] : null, seed2 = bracketData.seeds ? bracketData.seeds[p2] : null;

                    return (<div key={idx} className="relative flex flex-col space-y-12 mb-8"><div className={`h-10 border-b-2 ${w1 ? 'border-[#b35a38]' : 'border-slate-300'} flex justify-between items-end bg-white relative text-left`}><span className={`${w1 ? 'text-[#b35a38] font-black' : 'text-slate-700 font-bold'} text-sm uppercase truncate`}>{seed1 ? <span className="text-base text-orange-600 font-black mr-1">{seed1}.</span> : null}{p1 || ""}</span><span className="text-[#b35a38] font-black text-sm ml-3">{s1}</span></div><div className={`h-10 border-b-2 ${w2 ? 'border-[#b35a38]' : 'border-slate-300'} flex justify-between items-end relative bg-white text-left`}><span className={`${w2 ? 'text-[#b35a38] font-black' : 'text-slate-700 font-bold'} text-sm uppercase truncate`}>{seed2 ? <span className="text-base text-orange-600 font-black mr-1">{seed2}.</span> : null}{p2 || ""}</span><span className="text-[#b35a38] font-black text-sm ml-3">{s2}</span></div></div>);
                ...
                <div className="flex flex-col justify-around h-auto min-h-[600px] w-80 ml-32 relative text-left">
                  ...
                  {[0, 2, 4, 6].map((idx) => {
                    ...
                    const p1 = bracketData.isLarge ? bracketData.r3[idx] : bracketData.r2[idx], p2 = bracketData.isLarge ? bracketData.r3[idx+1] : bracketData.r2[idx+1], w1 = p1 && (bracketData.isLarge ? bracketData.r4.includes(p1) : bracketData.r3.includes(p1)), w2 = p2 && (bracketData.isLarge ? bracketData.r4.includes(p2) : bracketData.r3.includes(p2));

                    const s1 = bracketData.isLarge ? bracketData.s3[idx] : bracketData.s2[idx], s2 = bracketData.isLarge ? bracketData.s3[idx+1] : bracketData.s2[idx+1];

                    const seed1 = bracketData.seeds ? bracketData.seeds[p1] : null, seed2 = bracketData.seeds ? bracketData.seeds[p2] : null;

                    return (<div key={idx} className="relative flex flex-col space-y-32 mb-16"><div className={`h-12 border-b-2 ${w1 ? 'border-[#b35a38]' : 'border-slate-300'} flex justify-between items-end bg-white relative text-center`}><span className={`${w1 ? 'text-[#b35a38] font-black' : 'text-slate-700 font-bold'} text-base uppercase`}>{seed1 ? <span className="text-xl text-orange-600 font-black mr-1">{seed1}.</span> : null}{p1 || ""}</span><span className="text-[#b35a38] font-black text-lg ml-4">{s1}</span></div><div className={`h-12 border-b-2 ${w2 ? 'border-[#b35a38]' : 'border-slate-300'} flex justify-between items-end bg-white relative text-center`}><span className={`${w2 ? 'text-[#b35a38] font-black' : 'text-slate-700 font-bold'} text-base uppercase`}>{seed2 ? <span className="text-xl text-orange-600 font-black mr-1">{seed2}.</span> : null}{p2 || ""}</span><span className="text-[#b35a38] font-black text-lg ml-4">{s2}</span></div></div>);
        ...
        {showRankingCalc && (
            ...
            <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                ...
                <div className="bg-white rounded-3xl p-8 max-w-lg w-full shadow-2xl relative max-h-[80vh] overflow-y-auto">
                    ...
                    <div className="text-center mb-6"><Trophy className="w-12 h-12 text-orange-500 mx-auto mb-2" /><h3 className="text-2xl font-black uppercase text-slate-800">Cálculo de Puntos</h3><p className="text-sm text-slate-500 font-medium">Torneo: {navState.tournamentShort}</p></div>

                    <div className="bg-slate-50 rounded-xl border-2 border-slate-100 overflow-hidden"><table className="w-full text-left"><thead className="bg-[#b35a38] text-white"><tr><th className="p-3 font-bold text-sm uppercase tracking-wider">Jugador</th><th className="p-3 font-bold text-sm uppercase tracking-wider text-right">Puntos</th></tr></thead><tbody className="divide-y divide-slate-100">{calculatedRanking.map((p, i) => (<tr key={i} className="hover:bg-white transition-colors"><td className="p-3 font-bold text-slate-700 uppercase text-sm">{p.name}</td><td className="p-3 font-black text-orange-600 text-right">{p.points}</td></tr>))}</tbody></table></div>

                    <div className="mt-6 flex gap-4">

                        <Button onClick={() => { const text = calculatedRanking.map(p => `${p.name}\t${p.points}`).join('\n'); navigator.clipboard.writeText(text); alert("Copiado"); }} className="flex-1 bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold h-12 rounded-xl"><Copy className="mr-2 w-4 h-4" /> COPIAR TABLA</Button>

                        <Button onClick={() => { let mensaje = `*RANKING CALCULADO - ${navState.tournamentShort}*\n\n`; calculatedRanking.forEach(p => { mensaje += `${p.name}: ${p.points}\n`; }); window.open(`https://wa.me/${MI_TELEFONO}?text=${encodeURIComponent(mensaje)}`, '_blank'); }} className="flex-1 bg-green-600 hover:bg-green-700 text-white font-bold h-12 rounded-xl"><Send className="mr-2 w-4 h-4" /> ENVIAR POR WHATSAPP</Button>
