...
const MI_TELEFONO = "5491150568353"; // <--- PONÉ TU NÚMERO ACÁ (ej: 5491122334455)
...
export default function Home() {
  ...
  const fetchGroupPhase = async (categoryShort: string, tournamentShort: string) => {
    ...
    try {
      ...
      setNavState({ ...navState, level: "tournament-phases" });
  ...
  const confirmarYEnviar = () => {
    let mensaje = `*SORTEO CONFIRMADO - ${navState.tournament}*\n*Categoría:* ${navState.categoryShort}\n\n`;
    groupData.forEach(g => {
      mensaje += `*${g.groupName}*\n${g.players.join('\n')}\n\n`;
    });
  ...
  const GroupTable = ({ group }: { group: any }) => (
    <div className="bg-white border-2 border-[#b35a38]/20 rounded-2xl overflow-hidden shadow-lg mb-4">
      <div className="bg-[#b35a38] p-3 text-white font-black italic text-center uppercase tracking-wider">
        {group.groupName}
      </div>
        ...
        <tbody>
          {group.players.map((p1: string, i: number) => (
            <tr key={i} className="border-b">
              <td className="p-3 border-r font-black bg-slate-50 uppercase text-[#b35a38]">{p1}</td>
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4 relative bg-[#fffaf5]">
      <div className={`w-full ${['direct-bracket', 'group-phase', 'ranking-view'].includes(navState.level) ? 'max-w-[95%]' : 'max-w-6xl'} mx-auto z-10`}>
        ...
        {isLoading && (
          <div className="fixed inset-0 bg-white/60 backdrop-blur-sm z-50 flex items-center justify-center text-center">
            ...
            <div className="flex justify-between items-center mb-8">
              ...
              {!isSorteoConfirmado && (
                <div className="flex space-x-2 text-center">
                  <Button onClick={() => runATPDraw(navState.categoryShort || navState.category, navState.tournamentShort)} size="sm" className="bg-orange-500 text-white font-bold"><RefreshCw className="mr-2 w-4 h-4" /> REHACER</Button>
                  <Button onClick={confirmarYEnviar} size="sm" className="bg-green-600 text-white font-bold px-8"><Send className="mr-2 w-4 h-4" /> CONFIRMAR Y ENVIAR</Button>
                </div>
              )}
            </div>

            <div className="bg-[#b35a38] p-6 rounded-3xl mb-12 text-center text-white italic text-center">
              <h2 className="text-3xl md:text-4xl font-black uppercase tracking-wider">{navState.tournament} - Fase de Grupos</h2>
              <p className="text-xs opacity-80 mt-1 font-bold uppercase">{navState.selectedCategory}</p>
            </div>
        ...
        {navState.level === "direct-bracket" && (
          <div className="bg-white border-2 border-[#b35a38]/10 rounded-[2.5rem] p-12 shadow-2xl overflow-x-auto min-h-[900px] text-center text-center">
        ...
        {navState.level === "ranking-view" && (
          <div className="bg-white border-2 border-[#b35a38]/10 rounded-[2.5rem] p-8 shadow-2xl overflow-hidden text-center text-center text-center text-center">
