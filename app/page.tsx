...
export default function Home() {
  ...
  // --- LÓGICA DE CARGA DE CUADRO (SUPER 8) ---
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-4 relative bg-[#fffaf5]">
      <div className={`w-full ${['ranking-view', 'group-phase', 'direct-bracket'].includes(navState.level) ? 'max-w-7xl' : 'max-w-6xl'} mx-auto z-10`}>
        ...
        <div className={`space-y-4 ${['ranking-view', 'group-phase', 'direct-bracket'].includes(navState.level) ? 'w-full' : 'max-w-xl mx-auto'}`}>
          ...
          {/* SECCIONES RESTANTES MANTENIDAS */}
                  ...
                  {[0, 2, 4, 6].map((idx) => {
                    ...
                    return (
                      <div key={idx} className="relative">
                        ...
                        {/* Conexión Recta Corregida */}
                        <div className="absolute top-[19px] -right-[48px] w-[48px] h-[55px] border-r-2 border-t-2 border-b-2 border-slate-300" />
                        <div className="absolute top-[46.5px] -right-[78px] w-8 h-[2px] bg-slate-300" />
                      </div>
                    )
                  })}
                </div>
                  ...
                  {[0, 2].map((idx) => {
                    ...
                    return (
                      <div key={idx} className="relative">
                        ...
                        <div className="absolute top-[19px] -right-[48px] w-[48px] h-[71px] border-r-2 border-t-2 border-b-2 border-slate-300" />
                ...
                <div className="flex flex-col items-center ml-24 w-80 relative z-10 text-center">
                  <div className="w-full space-y-12 mb-16">
                    {[0, 1].map((idx) => {
                      ...
                      return (
                        <div key={idx} className={`border-b-4 ${win ? 'border-[#b35a38]' : 'border-slate-200'} pb-2 h-8 bg-white flex justify-between items-end`}>
                  ...
                  <span className="text-[#b35a38] font-black text-3xl mt-2 italic uppercase tracking-tighter text-center block w-full">{bracketData.winner || "Campeón"}</span>
                </div>
              </div>
            </div>
          )}

          {/* Torneos Regulares, Grupos y Ranking (Intactos) */}
