...
export default function Home() {
  ...
  const buttonStyle = "w-full text-lg h-20 border-2 border-[#b35a38]/20 bg-white text-[#b35a38] hover:bg-[#b35a38] hover:text-white transform hover:scale-[1.01] transition-all duration-300 font-semibold shadow-md rounded-2xl text-center flex items-center justify-center";
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-4 relative bg-[#fffaf5]">
      <div className={`w-full ${['ranking-view', 'group-phase', 'direct-bracket'].includes(navState.level) ? 'max-w-7xl' : 'max-w-6xl'} mx-auto z-10`}>
        ...
        <div className={`space-y-4 ${['ranking-view', 'group-phase', 'direct-bracket'].includes(navState.level) ? 'w-full' : 'max-w-xl mx-auto'}`}>
          ...
          {/* Vistas de Ranking y Selección (Sin cambios) */}
          ...
          {/* CUADRO - SOLUCIÓN MECÁNICA DE UNIÓN */}
          {navState.level === "direct-bracket" && (
            <div className="bg-white border-2 border-[#b35a38]/10 rounded-[2.5rem] p-8 shadow-2xl overflow-x-auto animate-in fade-in duration-500">
              ...
              <div className="flex flex-row items-center min-w-[950px] max-w-6xl mx-auto py-10 relative">
                  ...
                  {[0, 2, 4, 6].map((idx) => {
                    ...
                    return (
                      <div key={idx} className="relative flex flex-col space-y-6">
                        ...
                        {/* Conector Vertical y Salida (UNIÓN SELLADA) */}
                        <div className="absolute top-[31px] bottom-[-1px] -right-[48px] w-[2px] bg-slate-300 flex items-center">
                          <div className="absolute left-0 w-8 h-[2px] bg-slate-300 -right-[30px] translate-x-[2px]" />
                        </div>
                      </div>
                    )
                  })}
                ...
                {/* Semis */}
                <div className="flex flex-col space-y-[158px] w-64 ml-24 relative z-10 text-left text-left">
                  {[0, 2].map((idx) => {
                    ...
                    return (
                      <div key={idx} className="relative flex flex-col space-y-10">
                        ...
                        {/* Conector Vertical y Salida Semis */}
                        <div className="absolute top-[31px] bottom-[-1px] -right-[48px] w-[2px] bg-slate-300 flex items-center">
                          <div className="absolute left-0 w-8 h-[2px] bg-slate-300 -right-[30px] translate-x-[2px]" />
                        </div>
                      </div>
                ...
                {/* Final */}
                <div className="flex flex-col items-center ml-24 w-80 relative z-10 text-center text-center text-center">
                  ...
                  <span className="text-[#b35a38] font-black text-3xl mt-2 italic uppercase tracking-tighter block w-full text-center text-center">{bracketData.winner || "Campeón"}</span>
                </div>
              </div>
            </div>
          )}
