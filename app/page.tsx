...
import { Trophy, Users, Grid3x3, RefreshCw, ArrowLeft, Trash2, Loader2, Send, AlertCircle, Shuffle, X, Copy, List, CheckCircle } from "lucide-react"
...
export default function Home() {
  ...
  const getTournamentStyle = (shortName: string) => {
      ...
      const map: any = { 
          "adelaide": "adelaide", "ao": "ao", "us open": "us", "us": "us", "indian wells": "iw", "iw": "iw", 
          "masters": "masters", "wimbledon": "wimbledon", "w": "wimbledon", "roland garros": "rg", "rg": "rg",
          "monte carlo": "mc", "mc": "mc", "s8 500": "s8_500", "super 8 / 500": "s8_500", "s8 250": "s8_250", "super 8 / 250": "s8_250"
      };
  ...
  const calculateAndShowRanking = async () => {
    ...
    try {
        ...
        if (colIndex === -1) { alert(`No se encontró la configuración de puntos para "${navState.tournamentShort}" en la fila 37 de Formatos Grupos.`); setIsLoading(false); return; }
        ...
        if (bracketData.hasData) {
            ...
            let semis: string[] = [], cuartos: string[] = [], octavos: string[] = [], dieciseis: string[] = [], finalists: string[] = [];
                            ...
                            if (groupRows[rowIndex]) {
                                for(let y=1; y<=players.length; y++) {
                                    ...
                                    if(res && res.length > 2) {
                                        ...
                                        let sW = 0, sL = 0;
                                        sets.forEach(s => { if(s.includes("/")) { const parts = s.split("/").map(Number); if(parts[0] > parts[1]) sW++; else sL++; } });
    ...
    } catch (e) { console.error(e); alert("Error calculando ranking. Verificá la hoja Formatos Grupos."); } finally { setIsLoading(false); }
  };
  ...
  const runDirectDraw = async (categoryShort: string, tournamentShort: string) => {
    ...
    try {
        ...
        if (filteredInscriptos.length < 4) { alert("Mínimo 4 jugadores para armar un cuadro."); setIsLoading(false); return; }
        ...
        for (let i = 0; i < bracketSize; i += 2) { if (slots[i] === null && slots[i+1] === null) emptyPairsIndices.push(i); }
        ...
        for (const player of nonSeeds) { const emptyTop = emptySlots.filter(i => i < bracketSize/2); const emptyBot = emptySlots.filter(i => i >= bracketSize/2); let targetIdx = -1; if (countTop <= countBot && emptyTop.length > 0) targetIdx = emptyTop[Math.floor(Math.random() * emptyTop.length)]; else if (emptyBot.length > 0) targetIdx = emptyBot[Math.floor(Math.random() * emptyBot.length)]; else if (emptyTop.length > 0) targetIdx = emptyTop[Math.floor(Math.random() * emptyTop.length)]; if (targetIdx !== -1) { slots[targetIdx] = player; if (targetIdx < bracketSize/2) countTop++; else countBot++; emptySlots = emptySlots.filter(i => i !== targetIdx); } }
        for (let i = 0; i < slots.length; i++) { if (slots[i] === null) slots[i] = { name: "BYE", rank: 0 }; }
        let matches = [];
        for (let i = 0; i < bracketSize; i += 2) { let p1 = slots[i]; let p2 = slots[i+1]; if (p1?.name === "BYE" && p2?.name !== "BYE") { let temp = p1; p1 = p2; p2 = temp; } matches.push({ p1, p2 }); }
  ...
  const runATPDraw = async (categoryShort: string, tournamentShort: string) => {
    ...
    try {
      ...
      const inscCsv = await inscRes.text();
      const filteredInscriptos = parseCSV(inscCsv).slice(1).filter(cols => cols[0] === tournamentShort && cols[1] === categoryShort).map(cols => cols[2]);
      if (filteredInscriptos.length === 0) { alert(`No hay inscriptos para ${tournamentShort} (${categoryShort}) en la pestaña Inscriptos.`); setIsLoading(false); return; }
      const entryList = filteredInscriptos.map(n => { const p = playersRanking.find(pr => pr.name.toLowerCase().includes(n.toLowerCase()) || n.toLowerCase().includes(pr.name.toLowerCase())); return { name: n, points: p ? p.total : 0 }; }).sort((a, b) => b.points - a.points);
      const totalPlayers = entryList.length;
      ...
      else { const remainder = totalPlayers % 3; if (remainder === 0) { groupsOf3 = totalPlayers / 3; } else if (remainder === 1) { groupsOf2 = 2; groupsOf3 = (totalPlayers - 4) / 3; } else if (remainder === 2) { groupsOf2 = 1; groupsOf3 = (totalPlayers - 2) / 3; } for(let i=0; i<groupsOf3; i++) capacities.push(3); for(let i=0; i<groupsOf2; i++) capacities.push(2); }
      ...
      // MODIFICACIÓN 1: Agregar ranking a cabezas de serie
      ...
      let pIdx = 0;
      for (let g = 0; g < numGroups; g++) { while (groups[g].players.length < groups[g].capacity && pIdx < restOfPlayers.length) { groups[g].players.push(restOfPlayers[pIdx].name); pIdx++; } }
      setGroupData(groups);
      ...
      if (res.ok && !csvText.includes("<!DOCTYPE html>") && (csvText.includes("Zona") || csvText.includes("Grupo"))) {
        ...
        for (let i = 0; i < rows.length; i++) {
          if (rows[i] && rows[i][0] && (rows[i][0].includes("Zona") || rows[i][0].includes("Grupo"))) {
            ...
            const validPlayersIndices: number[] = []; const players: string[] = []; const positions = []; const points = []; const diff = []; const gamesDiff = []; 
  ...
  const GroupTable = ({ group }: { group: any }) => {
    ...
    let isComplete = true;
    for (let i = 0; i < totalPlayers; i++) { for (let j = 0; j < totalPlayers; j++) { if (i === j) continue; const val = group.results[i]?.[j]; if (!val || val.trim() === "-" || val.trim().length < 2) { isComplete = false; break; } } if (isComplete) break; }
    const hasTies = () => { if (!group.points || !group.diff) return false; const signatures = group.points.map((p: any, i: number) => `${p}|${group.diff[i]}`); const unique = new Set(signatures); return unique.size !== signatures.length; };
    const showGames = hasTies();
    const calculateRanks = () => { return group.positions || []; };
    ...
    <div className={`bg-white border-2 border-opacity-20 rounded-2xl overflow-hidden shadow-lg mb-4 text-center h-fit overflow-hidden ${style.borderColor}`}>
      <div className={`${style.color} p-3 text-white font-black italic text-center uppercase tracking-wider relative flex items-center justify-center`}> <span className="text-3xl">{group.groupName}</span> </div>
            ...
            <thead>
              <tr className="bg-slate-50 border-b">
                ...
                {group.players && group.players.map((p: string, i: number) => {
                  let shortName = p;
                  if (p) { const clean = p.replace(/,/g, "").trim().split(/\s+/); if (clean.length > 1) shortName = `${clean[0]} ${clean[1].charAt(0)}.`; else shortName = clean[0]; }
                  return ( <th key={i} className={`p-3 border-r text-center font-black uppercase min-w-[80px] whitespace-nowrap ${style.textColor}`}> {shortName} </th> )
                })}
                {isComplete && ( <> <th className="p-2 text-center font-black text-slate-600 bg-slate-100 whitespace-nowrap">PTS</th> <th className="p-2 text-center font-black text-slate-600 bg-slate-100 whitespace-nowrap">SETS</th> {showGames && <th className="p-2 text-center font-black text-slate-600 bg-slate-100 whitespace-nowrap">GAMES</th>} <th className="p-3 text-center font-black text-black bg-slate-100 w-12 whitespace-nowrap">POS</th> </> )}
              </tr>
            </thead>
            <tbody>
              {group.players && group.players.map((p1: string, i: number) => (
                <tr key={i} className="border-b">
                  ...
                  {group.players.map((p2: string, j: number) => ( <td key={j} className={`p-2 border-r text-center font-black text-slate-700 whitespace-nowrap text-sm md:text-base ${i === j ? 'bg-slate-100 text-slate-300' : ''}`}> {i === j ? "/" : (group.results[i] && group.results[i][j] ? group.results[i][j] : "-")} </td> ))}
                  {isComplete && ( <> <td className="p-2 text-center font-bold text-slate-700 bg-slate-50">{group.points ? group.points[i] : "-"}</td> <td className="p-2 text-center font-bold text-slate-700 bg-slate-50">{group.diff ? group.diff[i] : "-"}</td> {showGames && <td className="p-2 text-center font-bold text-slate-700 bg-slate-50">{group.gamesDiff ? group.gamesDiff[i] : "-"}</td>} <td className={`p-3 text-center font-black text-xl bg-slate-50 whitespace-nowrap ${style.textColor}`}> {displayRanks[i] || "-"}° </td> </> )}
                </tr>
              ))}
            </tbody>
          </table>
  ...
  const generatePlayoffBracket = (qualifiers: any[]) => {
    ...
    if (totalPlayers <= 32) {
        let bracketSize = 8;
        if (totalPlayers > 16) bracketSize = 32; else if (totalPlayers > 8) bracketSize = 16; else if (totalPlayers > 4) bracketSize = 8; else bracketSize = 4; 
        const byeCount = bracketSize - totalPlayers;
        ...
        for(let i=0; i < byeCount; i++) { if(winners[i]) playersWithBye.add(winners[i].name); else if(runners[i - winners.length]) playersWithBye.add(runners[i - winners.length].name); }
        ...
        const idxTop = 0; const idxBottom = numMatches - 1; const idxMidTop = halfMatches - 1; const idxMidBottom = halfMatches; 
        ...
        matches.forEach((match, index) => {
            ...
            } else { if (pool.length >= 2) { match.p1 = pool.pop(); match.p2 = pool.pop(); } else if (pool.length === 1) { match.p1 = pool.pop(); match.p2 = { name: "BYE", rank: 0 }; } }
        });
        ...
        for(let i=0; i<assignedByesCount; i++) { if(winners[i]) priorityByeZones.add(winners[i].groupIndex); }
        let matches: any[] = Array(numMatches).fill(null).map(() => ({ p1: null, p2: null }));
        const placeP1 = (winner: any, index: number) => { if (!winner) return; matches[index].p1 = winner; if (priorityByeZones.has(winner.groupIndex)) { matches[index].p2 = { name: "BYE", rank: 0, groupIndex: -1 }; } };
        if (winners[0]) placeP1(winners[0], 0); 
        if (winners[1]) placeP1(winners[1], numMatches - 1); 
        const mids = [winners[2], winners[3]].filter(Boolean);
        if (mids.length > 0) { if (Math.random() > 0.5) mids.reverse(); if (mids[0]) placeP1(mids[0], (numMatches/2) - 1); if (mids[1]) placeP1(mids[1], (numMatches/2)); }
        const safeIndices = [7, 8, 23, 24].sort(() => Math.random() - 0.5);
        const z5to8 = winners.slice(4, 8); 
        z5to8.forEach(w => { if (safeIndices.length > 0) placeP1(w, safeIndices.pop()!); else { const emptyIdx = matches.findIndex(m => m.p1 === null); if (emptyIdx !== -1) placeP1(w, emptyIdx); } });
        ...
        remainingWinners.forEach(w => { if (emptyIndicesP1.length > 0) placeP1(w, emptyIndicesP1.pop()!); });
        const runnersTop = []; const runnersBot = []; const runnersFree = []; 
        runners.forEach(r => { const winnerMatchIndex = matches.findIndex(m => m.p1 && m.p1.groupIndex === r.groupIndex); if (winnerMatchIndex !== -1) { if (winnerMatchIndex < numMatches / 2) runnersBot.push(r); else runnersTop.push(r); } else { runnersFree.push(r); } });
        runnersTop.sort(() => Math.random() - 0.5); runnersBot.sort(() => Math.random() - 0.5); runnersFree.sort(() => Math.random() - 0.5);
        for(let i=0; i < numMatches/2; i++) { if (!matches[i].p1) { if(runnersTop.length > 0) matches[i].p1 = runnersTop.pop(); else if(runnersFree.length > 0) matches[i].p1 = runnersFree.pop(); } }
        for(let i=numMatches/2; i < numMatches; i++) { if (!matches[i].p1) { if(runnersBot.length > 0) matches[i].p1 = runnersBot.pop(); else if(runnersFree.length > 0) matches[i].p1 = runnersFree.pop(); } }
        ...
        remainingP1Slots.forEach(idx => { if (allRemainingRunners.length > 0) matches[idx].p1 = allRemainingRunners.pop(); else matches[idx].p1 = { name: "BYE", rank: 0 }; });
        ...
        emptyP2Indices.forEach(idx => { if (finalPool.length > 0) matches[idx].p2 = finalPool.pop(); else matches[idx].p2 = { name: "BYE", rank: 0 }; });
        matches.forEach(m => { if (!m.p1) m.p1 = { name: "BYE", rank: 0 }; if (!m.p2) m.p2 = { name: "BYE", rank: 0 }; if (m.p1.name === "BYE" && m.p2.name !== "BYE") { const temp = m.p1; m.p1 = m.p2; m.p2 = temp; } });
        return { matches, bracketSize };
    }
  }
  ...
  const confirmarSorteoCuadro = () => {
    ...
    // MODIFICACIÓN 3: Confirmación visual del cuadro
    setIsSorteoConfirmado(true);
  };
  ...
  const fetchBracketData = async (category: string, tournamentShort: string) => {
    ...
    try {
      ...
      if (hasContent) {
          ...
          let bracketSize = 16; 
          if (playersInCol1 > 16) bracketSize = 32; else if (playersInCol1 <= 8) bracketSize = 8; 
          let seeds = {};
          try {
          ...
          if (bracketSize === 32) winnerIdx = 10; else if (bracketSize === 16) winnerIdx = 8; else if (bracketSize === 8) winnerIdx = 6; 
      ...
      <div className={`w-full ${['direct-bracket', 'group-phase', 'ranking-view', 'damas-empty', 'generate-bracket'].includes(navState.level) ? 'max-w-[95%]' : 'max-w-6xl'} mx-auto z-10 text-center`}>
        ...
        {navState.level === "generate-bracket" && (
          <div className="bg-white border-2 border-[#b35a38]/10 rounded-[2.5rem] p-8 md:p-12 shadow-2xl text-center">
             ...
             {/* UI MODIFICADA: Botones de sorteo dinámicos y persistencia visual al confirmar */}
             {!isSorteoConfirmado ? (
                 <div className="flex flex-col md:flex-row gap-4 justify-center mt-8 sticky bottom-4 z-20">
                    ...
                    {tournaments.find(t => t.short === navState.tournamentShort)?.type === 'direct' ? (
                       // MODIFICACIÓN 2: Botón Sortear con color del torneo
                       <Button onClick={() => runDirectDraw(navState.category, navState.tournamentShort)} className={`${getTournamentStyle(navState.tournamentShort).color} text-white font-bold h-12 px-8 shadow-lg hover:opacity-90`}>
                           <Shuffle className="mr-2 w-4 h-4" /> Sortear
                       </Button>
                    ) : (
                       // MODIFICACIÓN 2: Botón Sortear con color del torneo
                       <Button onClick={() => fetchQualifiersAndDraw(navState.category, navState.tournamentShort)} className={`${getTournamentStyle(navState.tournamentShort).color} text-white font-bold h-12 px-8 shadow-lg hover:opacity-90`}>
                           <Shuffle className="mr-2 w-4 h-4" /> Sortear
                       </Button>
                    )}
                 ...
                 <div className="flex flex-col items-center justify-center mt-8 sticky bottom-4 z-20 space-y-4">
                     <div className="bg-green-100 border-2 border-green-500 text-green-800 px-8 py-3 rounded-xl font-bold flex items-center shadow-lg">
                         <CheckCircle className="mr-2 w-6 h-6" /> CUADRO CONFIRMADO
                     </div>
                     <Button onClick={goBack} className="bg-slate-500 text-white font-bold h-12 px-8 hover:bg-slate-600"><ArrowLeft className="mr-2" /> VOLVER AL MENÚ</Button>
                 </div>
             )}
          </div>
        )}
                ...
                {bracketData.canGenerate ? (
                    <div className="mt-4">
                        ...
                        <div className="flex gap-2 justify-center">
                            {tournaments.find(t => t.short === navState.tournamentShort)?.type === 'direct' ? (
                            <Button onClick={() => runDirectDraw(navState.category, navState.tournamentShort)} className={`${getTournamentStyle(navState.tournamentShort).color} text-white font-bold px-8 shadow-lg hover:opacity-90`}> <Shuffle className="mr-2 w-4 h-4" /> Sortear </Button>
                            ) : (
                            <Button onClick={() => fetchQualifiersAndDraw(navState.category, navState.tournamentShort)} className={`${getTournamentStyle(navState.tournamentShort).color} text-white font-bold px-8 shadow-lg hover:opacity-90`}> <Shuffle className="mr-2 w-4 h-4" /> Sortear </Button>
                            )}
                        </div>
                    </div>
...
.toLowerCase().includes("zona") || potentialP4.toLowerCase().includes("grupo") || potentialP4.includes("*"));
            const p4 = !isNextHeader && potentialP4 && potentialP4 !== "-" ? rows[i+4] : null;
            const playersRaw = [rows[i+1], rows[i+2], rows[i+3]];
            if (p4) playersRaw.push(p4);
            const validPlayersIndices: number[] = []; const players: string[] = []; const positions = []; const points = []; const diff = []; const gamesDiff = []; 
